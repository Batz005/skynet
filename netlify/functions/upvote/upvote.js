...
const handler = async (event, context) => {
  ...
  console.log(user_id, id, likes, "body")
...
mutation MyMutation($id: uuid!, $user_id: uuid, $is_liked: Boolean, $likes: Int) {
  update_Posts_by_pk(pk_columns: {id: $id},  _set: {likes: $likes}) {
    likes
  }
  insert_Post_Stats_one(object: {is_liked: $is_liked, post_id: $id, user_id: $user_id}){
    id  
    is_liked
      post_id
      user_id
...
mutation MyMutation($id: uuid!, $is_liked: Boolean, $likes: Int, $post_id: uuid!) {
    update_Post_Stats_by_pk(pk_columns: {id: $id}, _set: {is_liked: $is_liked}){
      id
      post_id
      is_liked
      is_disliked
    }
    update_Posts_by_pk(pk_columns: {id: $post_id},  _set: {likes: $likes}) {
        likes
      }
  }
...
function fetchMyQuery(id,user_id) {
  ...
  if(dataQuery?.Post_Stats.length !== 0){
      ...
      console.log(likes, is_liked, "136")
      if(is_liked){
          likes--;
      }
      else
